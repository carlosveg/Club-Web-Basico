...
const getDataAPI = async () => {
  const key = "XCe2rRk7qiPEGSrNxioNudgUuoyBNJy42FViZ8uc";
  const res = await fetch(`https://api.nasa.gov/planetary/apod?api_key=${key}`);
  const data = await res.json();
  const result = generarTarjeta(data);
  container.innerHTML = result;
};
const generarTarjeta = ({copyright, date, explanation, title, url}) => {
  return `
    <section class="card">
...
},{}]},["5Ff7C","5jkUb"], "5jkUb", "parcelRequirec754")
